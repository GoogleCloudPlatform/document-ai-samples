...
**Alternatively**, create the Cloud Function in the Cloud Console following these steps:
...
3. In the Environment field, select 1st gen. See [Cloud Functions version comparison](https://cloud.google.com/functions/docs/deploy#console:~:text=Cloud%20Functions%20version%20comparison) for a comparison.
4. Enter a Function name and select your intended region to deploy.
5. In the Trigger section, select the 'Cloud Storage' Trigger type. For event type, select 'On (finalizing/creating) file in the selected bucket'. Select the Bucket you created previously for document inputs. For demo purposes, leave 'Retry on failure' unchecked.
