...
for page in document.pages:
    for field in page.form_fields:
        ...
        names.append(
            trim_text(field.field_name.text_anchor.content))
        # Confidence - How "sure" the Model is that the text is correct
        name_confidence.append(field.field_name.confidence)

        values.append(
            trim_text(field.field_value.text_anchor.content))
        value_confidence.append(field.field_value.confidence)

# Create a Pandas Dataframe to print the values in tabular format.
df = pd.DataFrame({'Field Name': names, 'Field Name Confidence': name_confidence,
                  'Field Value': values, 'Field Value Confidence': value_confidence})
