...
async function updateForm() {
    for (const [field, pages] of Object.entries(gAnalysis)) {
        ...
        for (let i = 0; i < gPhotoMaxCount; ++i) {
            ...
            const page_confidence = (0 <= parseInt(page.confidence)) ? page.confidence : '-'
            confidences.push(`${page_confidence}%`)

            const page_value = (gParams.showNormalized && page.normalized)
                ? `📐 ${page.normalized}`
                : page.value
...
function resetAvatar(avatar) {
    ...
    if (confidenceIndex != -1)
        avatar.label.textContent = avatar.label.textContent.slice(0, confidenceIndex)
}
...
function updateSnapshotWithSample(index, filename) {
    const avatar = resetAvatarFromIndex(index)
    if (filename)
        avatar.image = `/api/samples/${filename}`
    updateAvatarZoom(avatar, index)
}
...
async function startVideo() {
    try {
        ...
        await eVideo.play()
        console.info(`Video resolution: ${eVideo.videoWidth}×${eVideo.videoHeight}`)
        return true
    } catch (error) {
        console.error(error)
        return false
...
async function onPhotoBlob(blob) {
    if (gPhotoIndex == 0)
        reset()  // Reset only for the first page (let users add other pages)
...
function selectFiles() {
    ...
    input.multiple = true
    input.accept = 'image/*'
    input.onchange = ev => { if (1 <= ev.target.files.length) onUserFiles(ev.target.files) }
    input.click()
}
...
function onSampleClick(event) {
    ...
    for (let i = 0; i < gPhotoMaxCount; ++i)
        resetAvatarFromIndex(i)

    const limit = Math.min(sampleFiles.length, gPhotoMaxCount)
    for (let i = 0; i < limit; ++i)
        updateSnapshotWithSample(i, sampleFiles[i])
}
...
async function initLocation() {
    ...
    for (const location of locations) {
        ...
        if (gParams.apiLocation == location)
            match = true
    }
...
async function initProcessors() {
    ...
    for (const [name, proc_data] of Object.entries(processors)) {
        ...
        if (gParams.processorData == proc_data)
            match = true
    }

    if (!match)
        gParams.processorData = null
    if (!gParams.processorData && 0 < eProcessor.children.length)
        gParams.processorData = eProcessor.children[0].value
    if (gParams.processorData)
        eProcessor.value = gParams.processorData
}
...
async function initForm() {
    ...
    for (const [index, field] of Object.entries(allFields))
        if (maxLabelChars < field.length)
            maxLabelChars = field.length
    ...
    for (const [index, field] of Object.entries(allFields)) {
        if (gImageFields.includes(field)) {
            ...
            if (labelOnLeft)
                imageField.className = 'label-on-left'
