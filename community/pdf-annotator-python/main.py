...
def main(args):
    ...
    for page_num, page_info in enumerate(document.pages):
        ...
        print(f"Found { len(page_info.form_fields)} form fields on page {page_num + 1}:")
        ...
        for field in page_info.form_fields:
            ...
            page_annotations.append(pikepdf.Dictionary(
    ...
    annotated_pdf.save(
        ...
        encryption=pikepdf.Encryption(
            owner='',
            user='',
            allow=pikepdf.Permissions(modify_annotation=False)
        )
    )
    print('Done.')
    return 0
...
def find_processor_id_of_type(
        client: DocumentProcessorServiceClient,
        parent: str,
        tartget_processor_type: str)-> str:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-i',
        '--input',
        help='filepath of input PDF to annotate',
        required=True)
    parser.add_argument(
        '--output',
        help='path to save annotated PDF')
    parser.add_argument(
        '--project-id',
        help='Project ID to use to call the Document AI API')
    parser.add_argument(
