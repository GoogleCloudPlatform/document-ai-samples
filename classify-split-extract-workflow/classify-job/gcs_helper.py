...
def get_list_of_uris(bucket_name: str, file_uri: str) -> List[str]:
  ...
  if filename != START_PIPELINE_FILENAME:
    ...
    if (mime_type and mime_type in MIME_TYPES) or (not mime_type and filename.lower().endswith(PDF_EXTENSION.lower())):
    ...
    for blob in blob_list:
      if blob.name and not blob.name.endswith('/') and \
            blob.name != START_PIPELINE_FILENAME and not os.path.dirname(blob.name).endswith(SPLITTER_OUTPUT_DIR):
        count += 1
