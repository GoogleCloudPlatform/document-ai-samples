...
def parse_results(request):
    ...
    if (len(blobs) > 1):
        ...
        document, page_image_names = merge_sharded_results(
            blobs, input_bucket_name, input_object_name, result_bucket_name, result_prefix)
        ...
        document_blob.upload_from_string(json.dumps(document), content_type="application/json")
        ...
        if not 'pages' in document or len(document['pages']) == 0 or not 'image' in document['pages'][0]:
            ...
            response['pageImageNames'] = response['pageImageNames'] + render_pdf_to_image(input_bucket_name=input_bucket_name, input_object_name=input_object_name, result_bucket_name=result_bucket_name, result_prefix=result_prefix)
        else:
            ...
            response['pageImageNames'] = response['pageImageNames'] + extract_image(document, input_bucket_name, input_object_name, result_bucket_name,result_prefix)
...
def check_entity_confidence(entities, entity_labels):
    ...
    for entity in entities:
        entity_label = [entity_label for entity_label in entity_labels if entity_label.label == entity.type]
...
def merge_sharded_results(blobs: List[storage.Blob], input_bucket_name, input_object_name, result_bucket_name, result_prefix):
    ...
    for blob in blobs:
        ...
        page_offset = min(int(page['pageNumber'])
        ...
        if 'shardInfo' in document:
            ...
            if 'shardCount' in shard_info:
                if shard_count != int(shard_info['shardCount']):
                    raise ValueError(f"Shard count mismatch! Current shard_count is {shard_count} shardInfo has {shard_info['shardCount']} and there are {len(blobs)} blobs with shard information")
            if 'shardIndex' in shard_info:
                shard_index = int(shard_info['shardIndex'])
                if shard_index >= shard_count:
                    raise ValueError(f"Shard index {shard_index} larger than or equal to shard count {shard_count}")
            else:
        ...
        page_image_names_per_shard[shard_index] = extract_image(document, input_bucket_name, input_object_name, result_bucket_name, result_prefix, page_offset)
    ...
    page_image_names = [item for sub_list in page_image_names_per_shard if sub_list for item in sub_list]
...
def list_results(bucket_name: str, prefix: str) -> List[storage.Blob]:
    ...
    """
    print(f"Retrieving blobs with processor results from bucket {bucket_name} with prefix {prefix}")
    storage_client = storage.Client()
...
def render_pdf_to_image(input_bucket_name, input_object_name, result_bucket_name, result_prefix, page_offset = None):
    page_image_names = []
    ...
    for i, image in zip(page_indices, renderer):
        ...
        image_blob_uri = f"gs://{result_bucket_name}/{Path(result_prefix,Path(image_filename).name)}"
...
def extract_image(document, input_bucket_name, input_object_name, result_bucket_name, result_prefix, page_offset = 0):
  page_image_names = []
  for page in document['pages']:
    if 'image' in page and 'content' in page['image']:
        ...
        page_blob.upload_from_string(base64.b64decode(
            page['image']['content']), content_type=mimetypes.types_map[extension])
    else:
        page_image_names = page_image_names + render_pdf_to_image(input_bucket_name=input_bucket_name, input_object_name=input_object_name, result_bucket_name=result_bucket_name, result_prefix=result_prefix, page_offset=page_offset)
  return page_image_names